...
import type { NlpIntentType } from '../ai/nlpRouter.js'

// ─── Risk Table ───────────────────────────────────────────────────────────────

export type RiskLevel = 'safe' | 'low' | 'medium' | 'high' | 'critical'

const RISK_MAP: Partial<Record<NlpIntentType, RiskLevel>> = {
...
export function getRiskLevel(intentType: NlpIntentType): RiskLevel {
 ...
 */
export function assessRisk(intentType: NlpIntentType): RiskLevel {
...
export async function confirmIfRequired(
  ...
  if (result.response === 1) return { proceed: true }
 ...
 */
export function riskWarningText(risk: RiskLevel, description: string): string | null {
  if (risk === 'medium') {
