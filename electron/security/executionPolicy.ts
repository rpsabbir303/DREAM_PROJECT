...
export function containsDangerousShellPattern(command: string): boolean {
...
export function isSafeTerminalCommand(command: string): boolean {
  ...
  if (!raw) return false
  if (containsDangerousShellPattern(raw)) return false
...
export function resolveWhitelistedApp(name: string): string | null {
