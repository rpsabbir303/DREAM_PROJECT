 ...
 */
function resolveAppKey(phrase: string): string | null {
  ...
  for (const [alias, key] of ALIAS_TO_KEY) {
    if (p === alias || p.startsWith(`${alias} `)) return key
  }
...
export function routeIntent(rawInput: string): ResolvedIntent | null {
  ...
  const result =
    matchSystem(s, rawInput)        // always highest priority (volume, screenshot …)
