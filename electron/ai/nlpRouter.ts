...
import { getRiskLevel, type RiskLevel } from '../security/actionGuard.js'
...
export type AppIntentType =
  | 'app.open' | 'app.close' | 'app.focus' | 'app.minimize'
  | 'app.maximize' | 'app.restore' | 'app.restart' | 'app.switch'

export type FileIntentType =
  | 'file.open' | 'file.create' | 'file.delete' | 'file.move'
  | 'file.rename' | 'file.search' | 'folder.open' | 'folder.create'

export type SystemIntentType =
  | 'system.volume' | 'system.brightness' | 'system.screenshot'
  | 'system.lock' | 'system.shutdown' | 'system.restart' | 'system.sleep'
  | 'system.wifi' | 'system.bluetooth' | 'system.recycle' | 'system.clipboard'

export type BrowserIntentType  = 'browser.url' | 'browser.search'
export type WindowIntentType   = 'window.list' | 'window.info'
export type AgentIntentType    = 'agent.plan'
export type KeyboardIntentType = 'keyboard.shortcut' | 'keyboard.type'

export type NlpIntentType =
  | AppIntentType | FileIntentType | SystemIntentType
  | BrowserIntentType | WindowIntentType | AgentIntentType
  | KeyboardIntentType
...
const DESKTOP_VERB_RE =
  /\b(open|launch|start|run|close|quit|exit|stop|switch\s+to|go\s+to|focus(?:\s+on)?|activate|minimize|minimise|maximize|maximise|restart|relaunch)\s+(.+)$/i
...
function peelToLeadingCommand(s: string): string {
...
function blocksGeminiFallback(intent: ResolvedIntent | null): boolean {
...
function stripVerb(s: string, re: RegExp): string | null {
  const m = s.match(re)
  if (!m) return null
...
function resolveKnownAppPhrase(phrase: string): string | null {
  const p = normalizeAppTokens(phrase.trim().toLowerCase())
  if (/^(what'?s?\s*app|what'?app|whats?\s*app|what\s+s\s+app|whatsapp|wa)$/i.test(p)) return 'whatsapp'
  if (/^(chrome|google\s*chrome|chorme|chrom)$/i.test(p)) return 'chrome'
  if (/^(vscode|vs\s*code|visual\s*studio\s*code)$/i.test(p)) return 'vscode'
  if (/^(notepad|note\s*pad)$/i.test(p)) return 'notepad'
  if (/^(explorer|file\s*explorer|files)$/i.test(p)) return 'explorer'
  return null
}

/** Windows discovery sometimes surfaces help/update strings as "apps". */
function isJunkDiscoveryName(canonicalName: string): boolean {
  const n = canonicalName.toLowerCase()
  if (/\b(what\s+is\s+new|latest\s+version|release\s+notes|getting\s+started)\b/i.test(n)) return true
...
function isPlausibleAppTarget(phrase: string): boolean {
  const p = phrase.trim().toLowerCase()
  if (!p || p.length < 2) return false
  if (/\b(is|are|was|will|can|should|would)\b/.test(p) && p.split(/\s+/).length >= 3) return false
  if (/\b(what\s+is|latest\s+version|new\s+in|is\s+new)\b/i.test(p)) return false
  if (/^what\s+app$/i.test(p)) return false   // use resolveKnownAppPhrase instead
  if (p.split(/\s+/).length > 4) return false
  return true
}

function resolveAppTarget(phrase: string): { appKey?: string; discoveryName?: string } | null {
  ...
  if (staticKey) return { appKey: staticKey }

  const fuzzy = fuzzyMatchAppKey(p, fuzzyThreshold)
  if (fuzzy) return { appKey: fuzzy.key }
 ...
 */
function looksLikeAppName(phrase: string): boolean {
  if (!phrase || phrase.length < 2 || phrase.length > 60) return false
...
function matchApp(s: string, raw: string): ResolvedIntent | null {
  let target: string | null

  const normTarget = (phrase: string) => normalizeAppTokens(phrase.trim())
  ...
  if ((target = stripVerb(s, V_OPEN)) !== null) {
    ...
    if (resolved) {
      ...
      const params: Record<string, string> = { app: target }
      if (appKey)        params.appKey        = appKey
      if (discoveryName) params.discoveryName = discoveryName
  ...
  if ((target = stripVerb(s, V_CLOSE)) !== null) {
    ...
    const resolved = resolveAppTarget(target)
    if (resolved) {
      const params: Record<string, string> = { app: target, ...(resolved.appKey ? { appKey: resolved.appKey } : {}) }
...
function matchFolderOpen(s: string, raw: string): ResolvedIntent | null {
...
function matchFile(s: string, raw: string): ResolvedIntent | null {
  // Create folder
...
function matchSystem(s: string, raw: string): ResolvedIntent | null {
  // Volume mute/unmute
  ...
  if (volSet) return make('system.volume', { action: 'set', level: volSet[1] }, raw, 0.95, `Volume → ${volSet[1]}%`)

  // Volume up
  const volUp = s.match(/^(?:increase|raise|turn up|volume up|louder)\s*(?:the\s+)?(?:volume)?\s*(?:by\s+(\d+)\s*%?)?$/)
  if (volUp) return make('system.volume', { action: 'increase', amount: volUp[1] ?? '5' }, raw, 0.93, 'Volume up')

  // Volume down
  const volDown = s.match(/^(?:decrease|lower|turn down|reduce|volume down|quieter|softer)\s*(?:the\s+)?(?:volume)?\s*(?:by\s+(\d+)\s*%?)?$/)
  if (volDown) return make('system.volume', { action: 'decrease', amount: volDown[1] ?? '5' }, raw, 0.93, 'Volume down')

  // Brightness
  const bright = s.match(/^(?:set\s+)?(?:the\s+)?brightness\s+(?:to\s+)?(\d{1,3})\s*%?$/)
  if (bright) return make('system.brightness', { level: bright[1] }, raw, 0.92, `Brightness → ${bright[1]}%`)
...
const SITE_RE = new RegExp(
  `^(?:open|go to|visit|navigate to|browse to)\\s+(?:(?:the|my)\\s+)?(?:website\\s+)?(?:site\\s+)?(${SITE_NAMES.map((n) => n.replace('.', '\\.')).join('|')})\\s*$`,
)
...
function matchBrowser(s: string, raw: string): ResolvedIntent | null {
...
function matchWindowInfo(s: string, raw: string): ResolvedIntent | null {
  if (/^(list|show|what'?s?)\s+(all\s+)?(open|running|active)\s+(apps?|applications?|windows?|programs?)/.test(s)) {
...
function matchContextual(s: string, raw: string): ResolvedIntent | null {
  // "what app am I using?" / "what am I using?" / "what's active?"
  if (
    /^what('?s?|'s| app| is| am i)? ?(am i using|is (open|active|focused|running|in front))?$/.test(s) ||
  ...
  if (
    /^close (it|this|current( app| window| tab)?|the current( app| window)?)$/.test(s) ||
    s === 'close current app' ||
  ...
  if (
    /^(switch back|go back|back to previous|previous app|switch to previous( app| window)?|go to previous)/.test(s) ||
    s === 'switch back' ||
    s === 'go back' ||
    s === 'previous app'
  ) {
  ...
  if (
    /^(reopen|re-open|open (it |that )?again|launch (it |that )?again|reopen (it|that))/.test(s) ||
    s === 'reopen it' ||
    s === 'reopen that' ||
    s === 'open it again'
  ) {
...
function matchAgent(s: string, raw: string): ResolvedIntent | null {
...
function matchDesktopHeuristic(s: string, raw: string): ResolvedIntent | null {
  ...
  if (openM) {
    ...
    if (resolved?.appKey || resolved?.discoveryName) {
      const params: Record<string, string> = { app: target }
      if (resolved.appKey) params.appKey = resolved.appKey
      if (resolved.discoveryName) params.discoveryName = resolved.discoveryName
  ...
  if (closeM) {
    ...
    const resolved = resolveAppTarget(target)
    if (resolved?.appKey) {
      const params: Record<string, string> = { app: target, appKey: resolved.appKey }
...
const KEY_PRESS_RE =
  /^(?:press|hit|tap|use|send|trigger|execute)\s+(.+)$/

const TYPE_RE =
  /^(?:type|write|input|enter|type\s+out)\s+(?:the\s+(?:text\s+)?)?[""']?(.+?)[""']?$/

/** Common shortcut name aliases so users don't need to know SendKeys syntax. */
const SHORTCUT_PHRASE_MAP: Record<string, string> = {
  'save':              'ctrl+s',
  'save file':         'ctrl+s',
  'save it':           'ctrl+s',
...
function matchKeyboard(s: string, raw: string): ResolvedIntent | null {
  ...
  if (pressM) {
    ...
    if (/^[a-z0-9+\-_ ]{1,30}$/.test(key) && (
      /[+]/.test(key) ||          // modifier combo: ctrl+s
...
function matchIntentChain(s: string, raw: string): ResolvedIntent | null {
  return (
    matchSystem(s, raw)
    ?? matchKeyboard(s, raw)
    ?? matchApp(s, raw)
 ...
 */
export function routeIntent(rawInput: string): ResolvedIntent | null {
  if (!rawInput?.trim() || rawInput.trim().length < 2) return null
  ...
  if (result) {
    safeLogger.info(
      `[JARVIS_NLP] resolved → type="${result.type}" confidence=${result.confidence} params=${JSON.stringify(result.params)} risk="${result.riskLevel}"`,
    )
...
export function isDesktopCommand(raw: string): boolean {
