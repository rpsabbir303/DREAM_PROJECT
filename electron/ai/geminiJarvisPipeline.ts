...
// ─── Layer 1: Structured intent extraction ────────────────────────────────────
//
// Asks Gemini to classify user input as one of four response shapes.
// This is the "AI brain" layer — it understands natural language and
// returns a typed intent for the automation engine to execute.

export type GeminiIntentType = 'DESKTOP_ACTION' | 'MULTI_STEP' | 'CLARIFY' | 'CHAT'

export interface GeminiDesktopAction {
  type: 'DESKTOP_ACTION'
  intent: string                      // e.g. "app.open"
  params: Record<string, string>      // e.g. { app: "whatsapp", appKey: "whatsapp" }
  confidence: number                  // 0–1
  requiresConfirmation: boolean
  displayLabel: string
}

export interface GeminiMultiStep {
  type: 'MULTI_STEP'
  goal: string                        // e.g. "work setup"
  steps: string[]                     // e.g. ["open chrome", "open slack"]
  displayLabel: string
}

export interface GeminiClarify {
  type: 'CLARIFY'
  question: string                    // e.g. "Which browser — Chrome or Edge?"
  candidates?: string[]
}
...
export async function extractStructuredIntent(
  userInput: string,
  context: { activeApp?: string; recentCommands?: string[] },
  signal: AbortSignal,
): Promise<GeminiExtractedIntent | null> {
  const contextHint = [
  ...
  try {
    ...
    const parsed = JSON.parse(trimmed) as GeminiExtractedIntent
