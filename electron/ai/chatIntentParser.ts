import type { AssistantIntentType, ChatMessage, CommandLogRecord, CommandUnderstanding } from '../../shared/interfaces/ipc.js'
import { parseIntent } from './intentParser.js'
import {
  estimateTargetAmbiguity,
  extractPathHintsFromHistory,
  formatRecentCommandsForPrompt,
  pickPrimaryContinuityHint,
...
const EXECUTABLE_INTENTS = new Set<AssistantIntentType>([
  'open_application',
  'open_url',
  'open_folder',
  'open_project',
...
function enrichUnderstanding(
  input: string,
  parsed: ReturnType<typeof parseIntent>,
  base: CommandUnderstanding,
  context?: { history: ChatMessage[]; recentCommands: CommandLogRecord[] },
  ...
  if (continuityHint) reasoningParts.push(`Continuity path candidate from chat: ${continuityHint}.`)
  if (recentLines.length) reasoningParts.push(`Recent commands (newest first): ${recentLines.slice(0, 3).join(' | ')}.`)
...
export function parseChatUnderstanding(
  input: string,
  context?: { history: ChatMessage[]; recentCommands: CommandLogRecord[] },
): CommandUnderstanding {
  ...
  if (EXECUTABLE_INTENTS.has(parsed.intent)) {
    const base: CommandUnderstanding = {
