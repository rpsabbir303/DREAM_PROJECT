...
export interface JarvisPersonaContext {
  /** High-level intent from the lightweight classifier (hints only for the model). */
  understanding: CommandUnderstanding
  /** Short memory lines from recent DB-backed chat (newest last). */
  memoryLines: string[]
  /** Optional note about execution that already ran (Ollama / rule path). */
  executionDigest?: string
  /** Decision-layer instructions (clarify / refuse / tool routing). */
  intelAddendum?: string
  /** Recent command log lines for continuity (newest first). */
  recentCommandLines?: string[]
  /** Implied editor / follow-up semantics from recent chat (non-path). */
  implicitContextLine?: string | null
}

/**
 * Core JARVIS system prompt: calm, concise, futuristic assistant tone.
 */
export function buildJarvisSystemPrompt(ctx: JarvisPersonaContext): string {
  const memoryBlock =
  ...
  const cmdBlock =
    ctx.recentCommandLines && ctx.recentCommandLines.length > 0
