import type { CommandUnderstanding } from '../../shared/interfaces/ipc.js'
import type { CommandIntelDecision, ToolRuntimeMode } from '../../shared/ai/jarvisCommandIntel.js'

function buildRefusalAddendum(): string {
  return [
...
function buildClarifyAddendum(understanding: CommandUnderstanding): string {
...
function buildVagueDeletionAddendum(): string {
  return [
...
function buildToolsEnabledAddendum(understanding: CommandUnderstanding): string {
  const parts = [
  ...
  if (understanding.ambiguity === 'medium' && understanding.actionRequired) {
    parts.push(
      'If you call a tool, prefix your reply with one short assumption (e.g. which folder you used) so the user can correct you.',
    )
  }
  if (understanding.continuityHint) {
    parts.push(
      `CONTINUITY: If the user refers to "the project", "that folder", or "here", prefer open_folder with "${understanding.continuityHint}" when it matches their goal.`,
    )
  }
  if (understanding.intent === 'run_safe_command' && understanding.continuityHint) {
    parts.push(
      `For run commands, the user may intend the project at "${understanding.continuityHint}" — only whitelisted commands exist; if none apply, tell them to run dev scripts from that folder in their own terminal.`,
    )
  }
  return parts.join(' ')
}

function summarizeDecision(mode: ToolRuntimeMode, understanding: CommandUnderstanding): string {
 ...
 */
export function resolveCommandIntelDecision(understanding: CommandUnderstanding, userInput: string): CommandIntelDecision {
  ...
  const vagueDestructive =
    /\b(delete|remove|erase|trash|wipe)\b/i.test(userInput) && /\b(file|folder|directories?|everything|old)\b/i.test(userInput)
  if (vagueDestructive) {
    return {
