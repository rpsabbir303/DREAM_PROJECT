import type { AssistantIntentType, MvpCommandKind, ParsedIntent } from '../../shared/interfaces/ipc.js'
...
function toMvpKind(intent: AssistantIntentType): MvpCommandKind {
  switch (intent) {
...
function build(intent: AssistantIntentType, target: string, confidence: number, rawInput: string): ParsedIntent {
...
export function parseIntent(input: string): ParsedIntent {
  ...
  if (lower.startsWith('open ') && lower.includes('project')) {
    ...
    if (!target) target = normalized.replace(/^open\s+/i, '').trim()
