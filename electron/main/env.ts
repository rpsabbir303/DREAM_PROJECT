 ...
 */
export function loadDesktopEnvironment(force = false): void {
  ...
  if (keyPresentButEmpty) {
    safeLogger.warn(
      '[JARVIS_ENV] GEMINI_API_KEY= is empty in .env — paste your key after the equals sign, save the file (Ctrl+S), then restart JARVIS.',
    )
  ...
  safeLogger.info(
    '[JARVIS_ENV] GEMINI_MODEL raw=',
    process.env.GEMINI_MODEL?.trim() ?? '(unset)',
    'resolved=',
    resolveGeminiModel(),
  )
}
...
export function reloadDesktopEnvironment(): void {
  envLoaded = false
