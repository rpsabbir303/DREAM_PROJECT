...
function formatCrossModal(hints: CrossModalReasoningHint[]): string {
...
export function buildUnifiedPromptMessages(params: {
  ...
  prioritizedSlices: PrioritizedContextSlice[]
  crossModalHints: CrossModalReasoningHint[]
  history: ChatMessage[]
  ...
  try {
    ...
    const modalitySections = prioritizedSlices.map((s) =>
      formatSection(`${s.chunk.title} [${s.chunk.kind}]`, s.chunk.content),
    )
