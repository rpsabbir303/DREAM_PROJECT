...
import type { MultimodalFusionPreferences, FusedContextChunk } from '../../shared/multimodal/types.js'
import { buildProductivityInsights } from '../productivity/productivityService.js'

const SCREEN_CONTEXT_TRIGGER =
  /(screen|visible|ui|dashboard|what.*see|window|overlay|ocr|screenshot|terminal output|error on my screen)/i

function shouldAttachScreenAnalyses(prefs: MultimodalFusionPreferences, userInput: string): boolean {
  if (prefs.includeScreenAnalyses === 'never') return false
  if (prefs.includeScreenAnalyses === 'always') return true
...
export async function fuseMultimodalContext(params: {
  ...
  cwd: string
}): Promise<{ chunks: FusedContextChunk[]; privacyNotes: string[] }> {
  const { memory, userInput, preferences, clientTerminalSnippet, cwd } = params
  const chunks: FusedContextChunk[] = []
