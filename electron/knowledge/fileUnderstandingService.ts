...
export async function collectProjectFiles(rootPath: string, maxFiles = 120): Promise<string[]> {
  const result: string[] = []
  ...
  while (queue.length > 0 && result.length < maxFiles) {
    ...
    for (const entry of entries) {
      ...
      if (TEXT_EXTENSIONS.has(ext)) result.push(fullPath)
...
export async function summarizeFile(filePath: string, rootPath: string): Promise<string | null> {
