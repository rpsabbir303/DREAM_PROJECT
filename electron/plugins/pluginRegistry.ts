...
import {
  ...
  getClipboard,
  clearClipboard,
} from './system/systemControlAgent.js'
...
async function executeFileIntent(intent: ResolvedIntent): Promise<PluginResult> {
  ...
  switch (intent.type as NlpIntentType) {
    ...
    case 'file.search': {
      ...
      if (results.length === 0) return { ok: true, message }
...
async function executeSystemIntent(intent: ResolvedIntent): Promise<PluginResult> {
  const p = intent.params

  switch (intent.type as NlpIntentType) {
    case 'system.volume': {
      const action = p.action ?? 'mute'
      if (action === 'mute')     return volumeMute()
      if (action === 'set')      return volumeSet(Number(p.level ?? 50))
      if (action === 'increase') return volumeIncrease(Number(p.amount ?? 5))
      if (action === 'decrease') return volumeDecrease(Number(p.amount ?? 5))
      return volumeMute()
    }
    ...
    case 'system.clipboard': {
      if (p.action === 'clear') return clearClipboard()
      return getClipboard()
    }
    default:
...
async function executeBrowserIntent(intent: ResolvedIntent): Promise<PluginResult> {
  const p = intent.params

  switch (intent.type as NlpIntentType) {
    case 'browser.url': {
      if (p.site) return openSite(p.site)
      return openUrl(p.url ?? '')
    }
    case 'browser.search': {
      const engine = (p.engine as 'google' | 'youtube' | 'bing' | 'github') ?? 'google'
      if (engine === 'youtube') return youtubeSearch(p.query ?? '')
...
async function executeWindowIntent(intent: ResolvedIntent): Promise<PluginResult> {
  switch (intent.type as NlpIntentType) {
    case 'window.list': {
      ...
      if (windows.length === 0) return { ok: true, message: 'No windows found.' }
    ...
    case 'window.info': {
      ...
      if (!active) return { ok: true, message: 'Could not detect the active window.' }
      return {
        ok:      true,
