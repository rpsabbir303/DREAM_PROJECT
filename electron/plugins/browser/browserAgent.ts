...
type SearchEngine = 'google' | 'youtube' | 'bing' | 'github'
...
export async function openSite(name: string): Promise<{ ok: boolean; message: string }> {
  const url = resolveSiteShortcut(name)
  if (!url) return { ok: false, message: `Unknown site "${name}". Try "open youtube.com" with a full domain.` }
  return openUrl(url)
}
...
export async function youtubeSearch(query: string): Promise<{ ok: boolean; message: string }> {
