...
export function registerIpcHandlers({
  ...
  ipcMain.handle(IPC_CHANNELS.aiChatStartStream, async (event, payload) => {
    ...
    if (!parsed.success) {
      const sid =
        payload && typeof payload === 'object' && 'streamId' in payload && typeof (payload as { streamId: unknown }).streamId === 'string'
  ...
  ipcMain.handle(IPC_CHANNELS.aiSetApiKey, async (_event, key: string) => {
    ...
    try {
      ...
      let content = ''
      try { content = readFileSync(envPath, 'utf-8') } catch { /* file may not exist yet */ }
