...
console.log('[JARVIS_PRELOAD] preload initialized')
...
const api: JarvisDesktopApi = {
  ...
  ai: {
    ...
    startChatStream: (input) => {
      console.info('[JARVIS_PRELOAD] startChatStream → main', input.streamId)
...
try {
  ...
  console.info('[JARVIS_PRELOAD] contextBridge exposed window.jarvis + window.electron (same API)')
  console.info('[JARVIS_PRELOAD] bridge top-level keys:', Object.keys(api).join(', '))
} catch (err) {
  console.error('[JARVIS_PRELOAD] contextBridge.exposeInMainWorld failed — renderer will have no IPC bridge', err)
}
