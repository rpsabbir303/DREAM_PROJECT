...
export interface ProcessInfo {
  name: string      // Image name (e.g. chrome.exe)
  pid: number
  sessionName: string
  memoryKb: number
  windowTitle?: string
...
export async function isProcessRunning(imageName: string): Promise<boolean> {
  try {
    const { stdout } = await execAsync(
      `cmd /c tasklist /FI "IMAGENAME eq ${imageName}" /NH`,
...
export async function detectByWindowTitle(title: string): Promise<string | null> {
  try {
...
export async function listRunningApps(): Promise<ProcessInfo[]> {
  try {
    ...
    const raw: Array<{ Name: string; Id: number; SessionName: string; WorkingSet: number }> =
...
export async function listWindowedProcesses(): Promise<Array<{ name: string; pid: number; title: string }>> {
  try {
    const ps = `
      Get-Process |
...
export type KillResult = { ok: boolean; message: string }

/** Kill a process by its image name. */
export async function killByImageName(imageName: string): Promise<KillResult> {
  try {
