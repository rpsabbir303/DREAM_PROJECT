...
function buildProcLookup(processNameOrTitle: string): string {
...
async function windowOp(processNameOrTitle: string, swCmd: number, opName: string): Promise<WindowOpResult> {
  try {
...
export async function focusWindow(processNameOrTitle: string): Promise<WindowOpResult> {
  try {
    const safe = processNameOrTitle.replace(/'/g, "\\'")
    const body = `
...
export async function minimizeWindow(processNameOrTitle: string): Promise<WindowOpResult> {
  return windowOp(processNameOrTitle, SW_MINIMIZE, `Minimized "${processNameOrTitle}"`)
}

export async function maximizeWindow(processNameOrTitle: string): Promise<WindowOpResult> {
  return windowOp(processNameOrTitle, SW_MAXIMIZE, `Maximized "${processNameOrTitle}"`)
}

export async function restoreWindow(processNameOrTitle: string): Promise<WindowOpResult> {
  return windowOp(processNameOrTitle, SW_RESTORE, `Restored "${processNameOrTitle}"`)
}

// ─── Window Listing (Phase 5) ─────────────────────────────────────────────────
