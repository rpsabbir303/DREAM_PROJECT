...
export interface ProcessNode {
  pid:       number
  name:      string      // image name without .exe
  parentPid: number | null
  exePath:   string | null
  /** PIDs of direct children (populated after full graph build) */
  children:  number[]
}

export type ProcessGraph = Map<number, ProcessNode>

// ─── State ────────────────────────────────────────────────────────────────────

let _graph: ProcessGraph = new Map()
...
async function fetchProcessGraph(): Promise<ProcessGraph> {
  try {
    ...
    const graph: ProcessGraph = new Map()
...
export function startProcessGraph(): void {
  if (_timer) return
...
export function stopProcessGraph(): void {
  if (_timer) { clearInterval(_timer); _timer = null }
}

/** Return the current process graph snapshot. */
export function getProcessGraph(): ProcessGraph {
  return _graph
}
 ...
 */
export function findRootPid(pid: number, maxDepth = 8): number {
  let current = pid
  let depth   = 0

  while (depth++ < maxDepth) {
    const node = _graph.get(current)
    if (!node?.parentPid) break

    const parent = _graph.get(node.parentPid)
    if (!parent) break

    // Stop at system processes
    if (['system', 'services', 'svchost', 'lsass', 'explorer'].includes(parent.name)) break
 ...
 */
export function getAppPidGroup(pid: number): number[] {
  const root  = findRootPid(pid)
  const group = new Set<number>()

  function collect(p: number): void {
    group.add(p)
 ...
 */
export function findPidsByName(processName: string): number[] {
  const target = processName.toLowerCase().replace(/\.exe$/i, '')
  const found: number[] = []
  for (const node of _graph.values()) {
    if (node.name === target) found.push(node.pid)
  }
  return found
}

/** True if any process with the given name is currently in the graph. */
export function isProcessInGraph(processName: string): boolean {
