...
export interface AppDefinition {
  /** Canonical display name shown to user */
  displayName: string
  /** Exe image names for tasklist/taskkill (priority order) */
  processNames: string[]
  /** Substring match for PowerShell window title fallback */
  windowTitle?: string
  /** `cmd /c <openCmd>` for standard Win32 apps */
  openCmd?: string
  /** shell:AppsFolder URI for UWP/Store apps (launched via explorer.exe) */
  explorerUri?: string
  /** Candidate full exe paths to check before PATH fallback */
  exePaths?: string[]
  /** Natural-language aliases for this app */
  aliases: string[]
}
...
const STATIC_REGISTRY: Record<AppKey, AppDefinition> = {
  // ── Browsers ──────────────────────────────────────────────────────────────
  chrome: {
...
const ALIAS_MAP = new Map<string, AppKey>()
...
let _discoveryCache: Map<string, DynamicApp> | null = null
let _discoveryTs = 0
const DISCOVERY_TTL_MS = 5 * 60 * 1000

async function getDiscoveredApps(): Promise<Map<string, DynamicApp>> {
  if (_discoveryCache && Date.now() - _discoveryTs < DISCOVERY_TTL_MS) {
    return _discoveryCache
  }

  const map = new Map<string, DynamicApp>()
...
export function resolveApp(name: string): AppDefinition | null {
  ...
  if (aliasKey) return STATIC_REGISTRY[aliasKey] ?? null
  ...
  for (const [alias, appKey] of ALIAS_MAP) {
    if (key.includes(alias)) return STATIC_REGISTRY[appKey] ?? null
  }
...
export async function resolveAppDynamic(name: string): Promise<{ displayName: string; exePath: string } | null> {
  const staticMatch = resolveApp(name)
  if (staticMatch) {
    // Find first existing exe from static
    for (const exePath of staticMatch.exePaths ?? []) {
      if (exePath && existsSync(exePath)) return { displayName: staticMatch.displayName, exePath }
    }
  }
...
export function displayName(app: AppDefinition | null, fallback: string): string {
  return app?.displayName ?? fallback.charAt(0).toUpperCase() + fallback.slice(1)
}

/** Check if exe exists among static candidates. */
export function findExePath(app: AppDefinition): string | null {
