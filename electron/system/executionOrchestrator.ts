...
import type { DiscoveredApp } from './appDiscovery.js'

// ─── Types ────────────────────────────────────────────────────────────────────

export type OrchestratorAction = 'open' | 'close' | 'focus' | 'minimize' | 'maximize' | 'restore' | 'switch'

export interface OrchestratorRequest {
  action: OrchestratorAction
  /** Human-readable query used for window matching */
  query:  string
  /** Resolved app descriptor (may be undefined for close/focus of running apps) */
...
export interface OrchestratorResult {
  ok:       boolean
  message:  string
  /** HWND of the target window if one was identified */
  hwnd?:    number
  /** PID of the launched/found process */
  pid?:     number
  /** Whether a new process was spawned (false = focused existing) */
  launched: boolean
}
 ...
 */
function spawnDetached(executable: string, args: string[]): Promise<number> {
 ...
 */
function appLabel(query: string, app?: DiscoveredApp | null): string {
  return app?.canonicalName ?? query
}

// ─── Open / Launch ────────────────────────────────────────────────────────────

async function handleOpen(req: OrchestratorRequest): Promise<OrchestratorResult> {
  const { query, app } = req
  ...
  if (existingWindow) {
    const state =
      existingWindow.isMinimized ? 'minimized' :
  ...
  const newWindow = await waitForWindow(
    (w) => {
      if (hwndsBefore.has(w.hwnd)) return false
      if (processHint && w.processName.includes(processHint)) return true
...
async function handleClose(req: OrchestratorRequest): Promise<OrchestratorResult> {
  const { query, app } = req
  const label = appLabel(query, app)
...
async function handleWindowOp(
  req:  OrchestratorRequest,
  opFn: (hwnd: number) => Promise<{ ok: boolean; message: string }>,
...
export async function launchApp(query: string, app: DiscoveredApp): Promise<OrchestratorResult> {
  return orchestrate({ action: 'open', query, app })
}

export async function closeApp(query: string, app?: DiscoveredApp | null): Promise<OrchestratorResult> {
  return orchestrate({ action: 'close', query, app })
}

export async function focusApp(query: string, app?: DiscoveredApp | null): Promise<OrchestratorResult> {
  return orchestrate({ action: 'focus', query, app })
}

export async function minimizeApp(query: string, app?: DiscoveredApp | null): Promise<OrchestratorResult> {
  return orchestrate({ action: 'minimize', query, app })
}

export async function maximizeApp(query: string, app?: DiscoveredApp | null): Promise<OrchestratorResult> {
  return orchestrate({ action: 'maximize', query, app })
}

export async function restoreApp(query: string, app?: DiscoveredApp | null): Promise<OrchestratorResult> {
