...
function dlog(...args: unknown[]): void {
  if (!QUIET_DISCOVERY) safeLogger.info(...args)
}
...
function buildAliases(name: string): string[] {
  ...
  if (noVer && noVer !== base) set.add(noVer)

  // Drop company prefix if 3+ words
  if (words.length >= 3) set.add(words.slice(1).join(' '))
  if (words.length >= 2) set.add(words.slice(0, -1).join(' '))
  if (words[0] && words[0].length >= 3) set.add(words[0])

  const acronym = words.map((w) => w[0]).join('')
  if (acronym.length >= 2 && acronym.length <= 5) set.add(acronym)

  const fused = words.join('')
  if (fused.length >= 4 && fused !== base) set.add(fused)
...
async function runPS(
  ...
  try {
    ...
    const { stdout, stderr } = await execAsync(
      ...
      { windowsHide: true, timeout: timeoutMs, maxBuffer: 10 * 1024 * 1024, encoding: 'utf8' },
    )
    ...
    dlog(
      `[JARVIS_PS] ${label} → ${outTrimmed.split('\n').length} lines` +
    ...
    if (stderr) derror(`[JARVIS_PS] ${label} stderr=${stderr.slice(0, 300)}`)
    if (stdout) dlog(`[JARVIS_PS] ${label} partial stdout=${stdout.slice(0, 200)}`)
...
async function scanUwpApps(): Promise<DiscoveredApp[]> {
  ...
  for (const line of stdout.split('\n')) {
    ...
    if (!display || display.length < 2 || !familyName) continue
  ...
  if (apps.length === 0) {
    try {
      ...
      for (const line of tl.split('\n')) {
        ...
        if (!procName || procName === 'System Idle Process') continue
        const key = procName.toLowerCase()
        if (seen.has(key)) continue
        seen.add(key)
        if (title === 'N/A' || !title) continue   // background-only
...
function mergeRegistry(
  ...
  const lists: Record<AppSource, DiscoveredApp[]> = {
...
async function diagnosePSEnvironment(): Promise<void> {
  ...
  try {
    ...
    safeLogger.error(`[JARVIS_DIAG] where powershell FAILED: ${(e as Error).message}`)
  }
  ...
  // 4. Get-Process works?
  const gpResult = await runPS('diag:getprocess',
    '$p = Get-Process -ErrorAction SilentlyContinue\nWrite-Output "PROC_COUNT=$($p.Count)"',
    8_000,
  )
...
`, 8_000)
  ...
  // 6. Registry access?
  const regDiag = await runPS('diag:registry',
    '$r = Get-ItemProperty "HKLM:\\Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\*" -ErrorAction SilentlyContinue; Write-Output "REG_COUNT=$(@($r).Count)"',
    8_000,
  )
  ...
  try {
    ...
    safeLogger.error(`[JARVIS_DIAG] ✗ tmpdir NOT writable: ${(e as Error).message}`)
  }
...
async function runFullScan(): Promise<void> {
  ...
  if (smResult.status  === 'rejected') dwarn('[JARVIS_DISCOVERY] startMenu threw:', smResult.reason)
  if (regResult.status === 'rejected') dwarn('[JARVIS_DISCOVERY] registry threw:', regResult.reason)
  if (uwpResult.status === 'rejected') dwarn('[JARVIS_DISCOVERY] uwp threw:', uwpResult.reason)
  if (runResult.status === 'rejected') dwarn('[JARVIS_DISCOVERY] running threw:', runResult.reason)

  _registry = mergeRegistry(startMenu, registry, uwp, running)
  _indexed  = true

  safeLogger.info(
    `[JARVIS_DISCOVERY] scan completed — indexed ${_registry.length} apps ` +
...
export async function fuzzyResolve(query: string, minScore = 55): Promise<ResolvedDiscovery | null> {
  ...
  if (/^what\s*s?\s*app$|^whatsapp$|^wa$/.test(qRaw.replace(/[^\w\s]/g, ' ').replace(/\s+/g, ' ').trim())) {
    ...
    if (wa) return { app: wa, score: 100, matchReason: 'whatsapp_typo' }
  }
...
export function getDiscoveryRegistry(): DiscoveredApp[] { return _registry }
export function isDiscoveryReady(): boolean            { return _indexed }
export function getDiscoveryCount(): number            { return _registry.length }

/** Dump top N entries for the debug panel. */
export function dumpDiscoveredApps(limit = 50): {
  name: string; source: AppSource; exe: string | null; processName: string | null
}[] {
...
function scoreMatch(query: string, candidate: DiscoveredApp): { score: number; reason: string } {
  ...
  for (const t of targets) {
    ...
    if (t.includes(q)   && q.length >= 4 && !q.includes(' ')) return { score: 75,  reason: `contains:${t}` }
    if (q.includes(t)   && t.length >= 4 && !q.includes(' ')) return { score: 72,  reason: `contained_in:${t}` }
  }
  ...
  if (qWords.length === 0) return { score: 0, reason: 'stopwords_only' }
  const matched   = qWords.filter((qw) =>
    qw.length >= 4 && nameWords.some((nw) => nw.startsWith(qw) || (qw.startsWith(nw) && nw.length >= 4)),
  )
  ...
  if (!q.includes(' ') && q.length >= 4) {
    for (const t of targets) {
      ...
      if (dist <= 2) return { score: Math.round((1 - dist / maxLen) * 60), reason: `edit:${t}(${dist})` }
    }
  }
