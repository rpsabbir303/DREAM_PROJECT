...
export interface TrackedWindow {
  /** Win32 HWND as a JS number (64-bit int truncated to safe range) */
  hwnd:        number
  /** OS-assigned window title */
  title:       string
  /** OS process ID */
  pid:         number
  /** Image name without .exe, lower-cased (e.g. "chrome") */
  processName: string
  /** Full path to the executable, or null if unavailable */
  exePath:     string | null
  /** True when this is the system foreground window */
  isFocused:   boolean
  /** True when the window is minimised (IsIconic) */
  isMinimized: boolean
  /** True when the window is maximised (IsZoomed) */
  isMaximized: boolean
  /** Unix ms timestamp of last snapshot */
  capturedAt:  number
}

// ─── State ────────────────────────────────────────────────────────────────────

let _windows:  TrackedWindow[] = []
let _focused:  TrackedWindow | null = null
...
function parseSnapshot(stdout: string): TrackedWindow[] {
  const now     = Date.now()
  ...
  for (const raw of stdout.trim().split('\n')) {
    ...
    if (!hwnd || !pid || !title?.trim()) continue
...
async function runFullSnapshot(): Promise<void> {
  try {
    ...
    if (_focused) {
      const windowState: WindowSnapshot['windowState'] =
        _focused.isMinimized ? 'minimized' :
      ...
      if (!prevFocused || prevFocused.hwnd !== _focused.hwnd) {
        const fromSnap: WindowSnapshot | null = prevFocused ? {
...
async function runFocusCheck(): Promise<void> {
  try {
    ...
    for (const w of _windows) w.isFocused = (w.hwnd === hwnd)
    ...
    if (nowFocused?.hwnd !== _focused?.hwnd) {
      ...
      const fromSnap2: WindowSnapshot | null = prev ? {
...
export function startDesktopStateEngine(): void {
...
export function stopDesktopStateEngine(): void {
...
export function getTrackedWindows(): TrackedWindow[] {
  return _windows
}

/** The window that currently has focus, or null. */
export function getFocusedWindow(): TrackedWindow | null {
  return _focused
}

/** Find windows whose process name matches (partial, case-insensitive). */
export function findWindowsByProcess(processName: string): TrackedWindow[] {
...
export function findWindowsByTitle(fragment: string): TrackedWindow[] {
 ...
 */
export async function forceRefresh(): Promise<TrackedWindow[]> {
...
export async function waitForWindow(
  predicate:  (w: TrackedWindow) => boolean,
  timeoutMs = 5_000,
  pollMs    = 500,
