...
import {
  runtimeState as _runtimeStateRef,
  setActiveWindow,
  setRunningApps,
  type WindowSnapshot,
  type RunningApp,
...
let activeWindowTimer: ReturnType<typeof setInterval> | null = null
let runningAppsTimer:  ReturnType<typeof setInterval> | null = null

// ─── Process-name → friendly name resolution ─────────────────────────────────

const PROCESS_FRIENDLY: Record<string, string> = {
  'chrome.exe':          'Chrome',
  'msedge.exe':          'Edge',
  'firefox.exe':         'Firefox',
  'brave.exe':           'Brave',
...
function friendlyName(processName: string): string {
 ...
 */
export function isCachedRunning(processName: string): boolean {
  const key = processName.toLowerCase().replace(/\.exe$/, '')
  // Import runtimeState at the top of the file — avoid require() in ESM
  return _runtimeStateRef.runningApps.some((a) =>
    a.processName.toLowerCase().replace(/\.exe$/, '') === key,
  )
}
