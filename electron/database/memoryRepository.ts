...
export function createMemoryRepository(): MemoryRepository {
  ...
  const insert = db.prepare(
    'INSERT INTO command_logs (id, command, result, created_at) VALUES (?, ?, ?, ?)',
  )
  const getRecent = db.prepare(
    'SELECT id, command, result, created_at as createdAt FROM command_logs ORDER BY created_at DESC LIMIT ?',
  )
  const insertChatMessage = db.prepare(
    'INSERT INTO chat_messages (id, role, content, created_at) VALUES (?, ?, ?, ?)',
  )
  const insertTask = db.prepare(
    'INSERT INTO tasks (id, title, intent, target, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)',
  )
  const updateTask = db.prepare('UPDATE tasks SET status = ?, updated_at = ? WHERE id = ?')
  const getRecentTasks = db.prepare(
    'SELECT id, title, intent, target, status, created_at as createdAt, updated_at as updatedAt FROM tasks ORDER BY created_at DESC LIMIT ?',
  )
  const insertActivityLog = db.prepare(
    'INSERT INTO activity_logs (id, level, message, created_at) VALUES (?, ?, ?, ?)',
  )
  const getRecentActivityLogs = db.prepare(
    'SELECT id, level, message, created_at as createdAt FROM activity_logs ORDER BY created_at DESC LIMIT ?',
  )
  const getRecentConversations = db.prepare(
    'SELECT id, role, content, created_at as createdAt FROM chat_messages ORDER BY created_at DESC LIMIT ?',
  )
  ...
  `)
  const getCommandMemoryStats = db.prepare(
    'SELECT command, usage_count as usageCount, last_used_at as lastUsedAt, is_favorite as isFavorite FROM command_memory ORDER BY usage_count DESC LIMIT ?',
  )
  const getWorkflows = db.prepare(
    'SELECT id, name, description, steps_json as stepsJson, created_at as createdAt, updated_at as updatedAt FROM workflows ORDER BY updated_at DESC',
  )
  const insertWorkflow = db.prepare(
    'INSERT INTO workflows (id, name, description, steps_json, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)',
  )
  const workflowCount = db.prepare('SELECT COUNT(*) as total FROM workflows')
  const getProjects = db.prepare(
    'SELECT id, name, folder_path as folderPath, startup_commands_json as startupCommandsJson, applications_json as applicationsJson, updated_at as updatedAt FROM projects ORDER BY updated_at DESC',
  )
  const insertProject = db.prepare(
    'INSERT INTO projects (id, name, folder_path, startup_commands_json, applications_json, updated_at) VALUES (?, ?, ?, ?, ?, ?)',
  )
  const projectCount = db.prepare('SELECT COUNT(*) as total FROM projects')
  const getSuggestions = db.prepare(
    'SELECT id, message, confidence, created_at as createdAt FROM personalization_suggestions ORDER BY created_at DESC LIMIT ?',
  )
  const insertSuggestion = db.prepare(
    'INSERT INTO personalization_suggestions (id, message, confidence, created_at) VALUES (?, ?, ?, ?)',
  )
  const suggestionCount = db.prepare('SELECT COUNT(*) as total FROM personalization_suggestions')
  const getWorkflowSchedules = db.prepare(
    'SELECT id, workflow_id as workflowId, schedule_type as scheduleType, time_of_day as timeOfDay, day_of_week as dayOfWeek, run_at as runAt, is_enabled as isEnabled, last_run_at as lastRunAt, created_at as createdAt FROM workflow_schedules ORDER BY created_at DESC',
  )
  const insertWorkflowSchedule = db.prepare(
    'INSERT INTO workflow_schedules (id, workflow_id, schedule_type, time_of_day, day_of_week, run_at, is_enabled, last_run_at, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
  )
  const updateScheduleLastRun = db.prepare(
    'UPDATE workflow_schedules SET last_run_at = ? WHERE id = ?',
  )
  const getWorkflowRuns = db.prepare(
    'SELECT id, workflow_id as workflowId, workflow_name as workflowName, status, started_at as startedAt, completed_at as completedAt, message FROM workflow_runs ORDER BY started_at DESC LIMIT ?',
  )
  const insertWorkflowRun = db.prepare(
    'INSERT INTO workflow_runs (id, workflow_id, workflow_name, status, started_at, completed_at, message) VALUES (?, ?, ?, ?, ?, ?, ?)',
  )
  const insertScreenCapture = db.prepare(
    'INSERT INTO screen_captures (id, image_base64, width, height, source, created_at) VALUES (?, ?, ?, ?, ?, ?)',
  )
  const insertScreenAnalysis = db.prepare(
    'INSERT INTO screen_analyses (id, summary, ocr_text, confidence, active_window_json, created_at) VALUES (?, ?, ?, ?, ?, ?)',
  )
  const getRecentScreenAnalyses = db.prepare(
    'SELECT id, summary, ocr_text as ocrText, confidence, active_window_json as activeWindowJson, created_at as createdAt FROM screen_analyses ORDER BY created_at DESC LIMIT ?',
  )
  const insertAgentPlan = db.prepare(
    'INSERT INTO agent_plans (id, goal, reasoning, state, steps_json, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)',
  )
  const updateAgentPlanStateStmt = db.prepare(
    'UPDATE agent_plans SET state = ?, updated_at = ? WHERE id = ?',
  )
  const getAgentPlansStmt = db.prepare(
    'SELECT id, goal, reasoning, state, steps_json as stepsJson, created_at as createdAt, updated_at as updatedAt FROM agent_plans ORDER BY created_at DESC LIMIT ?',
  )
  const insertAgentRun = db.prepare(
    'INSERT INTO agent_runs (id, plan_id, goal, state, started_at, completed_at, message) VALUES (?, ?, ?, ?, ?, ?, ?)',
  )
  const getAgentRunsStmt = db.prepare(
    'SELECT id, plan_id as planId, goal, state, started_at as startedAt, completed_at as completedAt, message FROM agent_runs ORDER BY started_at DESC LIMIT ?',
  )
  ...
  const insertAiProviderMetric = db.prepare(
    'INSERT INTO ai_provider_logs (id, provider, model, latency_ms, input_chars, output_chars, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)',
  )
  const getAiProviderMetricsStmt = db.prepare(
    'SELECT provider, model, latency_ms as latencyMs, input_chars as inputChars, output_chars as outputChars, created_at as createdAt FROM ai_provider_logs ORDER BY created_at DESC LIMIT ?',
  )
  ...
  const getMemoryEmbeddingsStmt = db.prepare(
    'SELECT id, kind, content, vector_json as vectorJson, created_at as createdAt FROM memory_embeddings ORDER BY created_at DESC LIMIT ?',
  )
  const insertWorkspaceContextStmt = db.prepare(
    'INSERT INTO workspace_context_logs (id, app, title, process_name, timestamp) VALUES (?, ?, ?, ?, ?)',
  )
  const getLatestWorkspaceContextStmt = db.prepare(
    'SELECT app, title, process_name as processName, timestamp FROM workspace_context_logs ORDER BY timestamp DESC LIMIT 1',
  )
  ...
  const getKnowledgeChunksStmt = db.prepare(
    'SELECT id, source_type as sourceType, source_ref as sourceRef, content, metadata_json as metadataJson, embedding_json as embeddingJson, indexed_at as indexedAt FROM knowledge_chunks ORDER BY indexed_at DESC LIMIT ?',
  )
  ...
  const getKnowledgeGraphEdgesStmt = db.prepare(
    'SELECT id, from_node_id as fromNodeId, to_node_id as toNodeId, relation FROM knowledge_graph_edges ORDER BY updated_at DESC LIMIT ?',
  )
  const insertObservabilityEventStmt = db.prepare(
    'INSERT INTO observability_events (id, type, source, severity, title, message, metadata_json, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
  )
  const getRecentObservabilityEventsStmt = db.prepare(
    'SELECT id, type, source, severity, title, message, metadata_json as metadataJson, created_at as createdAt FROM observability_events ORDER BY created_at DESC LIMIT ?',
  )
  const insertProactiveNotificationStmt = db.prepare(
    'INSERT INTO proactive_notifications (id, level, title, message, event_id, action_label, created_at, read_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
  )
  const getRecentProactiveNotificationsStmt = db.prepare(
    'SELECT id, level, title, message, event_id as eventId, action_label as actionLabel, created_at as createdAt, read_at as readAt FROM proactive_notifications ORDER BY created_at DESC LIMIT ?',
  )
  const markProactiveNotificationReadStmt = db.prepare(
    'UPDATE proactive_notifications SET read_at = ? WHERE id = ?',
  )
  const insertLearningFeedbackStmt = db.prepare(
    'INSERT INTO learning_feedback (id, type, source, action, outcome, score, metadata_json, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
  )
  const getLearningFeedbackStmt = db.prepare(
    'SELECT id, type, source, action, outcome, score, metadata_json as metadataJson, created_at as createdAt FROM learning_feedback ORDER BY created_at DESC LIMIT ?',
  )
  ...
  const getBehaviorPatternsStmt = db.prepare(
    'SELECT id, name, description, category, confidence, frequency, last_seen_at as lastSeenAt, related_actions_json as relatedActionsJson FROM behavior_patterns ORDER BY confidence DESC, frequency DESC LIMIT ?',
  )
  const insertAdaptiveRecommendationStmt = db.prepare(
    'INSERT INTO adaptive_recommendations (id, title, message, category, confidence, impact_score, source_pattern_id, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
  )
  const getAdaptiveRecommendationsStmt = db.prepare(
    'SELECT id, title, message, category, confidence, impact_score as impactScore, source_pattern_id as sourcePatternId, status, created_at as createdAt FROM adaptive_recommendations ORDER BY created_at DESC LIMIT ?',
  )
  const setAdaptiveRecommendationStatusStmt = db.prepare(
    'UPDATE adaptive_recommendations SET status = ? WHERE id = ?',
  )
  const insertWorkflowOptimizationInsightStmt = db.prepare(
    'INSERT INTO workflow_optimization_insights (id, workflow_id, workflow_name, issue, suggestion, confidence, estimated_impact, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
  )
  const getWorkflowOptimizationInsightsStmt = db.prepare(
    'SELECT id, workflow_id as workflowId, workflow_name as workflowName, issue, suggestion, confidence, estimated_impact as estimatedImpact, created_at as createdAt FROM workflow_optimization_insights ORDER BY created_at DESC LIMIT ?',
  )
  ...
  function deserializeWorkflow(row: {
    ...
    return {
      ...
      steps: JSON.parse(row.stepsJson) as WorkflowDefinition['steps'],
    }
  }
  ...
  function deserializeProject(row: {
    ...
    return {
      ...
      startupCommands: JSON.parse(row.startupCommandsJson) as string[],
      applications: JSON.parse(row.applicationsJson) as string[],
      updatedAt: row.updatedAt,
    }
  }
  ...
  return {
    ...
    getRecentCommands(limit = 20) {
      return getRecent.all(limit) as CommandLogRecord[]
    },
    ...
    getRecentTasks(limit = 30) {
      return getRecentTasks.all(limit) as AssistantTask[]
    },
    ...
    getRecentActivityLogs(limit = 100) {
      return getRecentActivityLogs.all(limit) as ActivityLogRecord[]
    },
    getRecentConversations(limit = 24) {
      return getRecentConversations.all(limit) as ChatMessage[]
    },
    upsertCommandMemory(command) {
      upsertCommandMemory.run(command, new Date().toISOString())
    },
    getCommandMemoryStats(limit = 20) {
      return (getCommandMemoryStats.all(limit) as Array<{
        command: string
        usageCount: number
        lastUsedAt: string
        isFavorite: number
      }>).map((item) => ({ ...item, isFavorite: item.isFavorite === 1 }))
    },
    getWorkflows() {
      return (getWorkflows.all() as Array<{
        id: string
        name: string
        description: string
        stepsJson: string
    ...
    getProjects() {
      return (getProjects.all() as Array<{
        id: string
        name: string
        folderPath: string
        startupCommandsJson: string
    ...
    getPersonalizationSuggestions(limit = 12) {
      return getSuggestions.all(limit) as PersonalizationSuggestion[]
    },
    ...
    getWorkflowRuns(limit = 40) {
      return getWorkflowRuns.all(limit) as WorkflowRunRecord[]
    },
    getWorkflowSchedules() {
      return (getWorkflowSchedules.all() as Array<
        Omit<WorkflowSchedule, 'isEnabled'> & { isEnabled: number }
      >).map((item) => ({ ...item, isEnabled: item.isEnabled === 1 }))
    },
    ...
    addScreenAnalysis(analysis) {
      insertScreenAnalysis.run(
        analysis.id,
        analysis.summary,
        analysis.ocrText,
        analysis.confidence,
        analysis.activeWindow ? JSON.stringify(analysis.activeWindow) : null,
        analysis.createdAt,
      )
    },
    getRecentScreenAnalyses(limit = 24) {
      return (getRecentScreenAnalyses.all(limit) as Array<{
        id: string
        summary: string
        ocrText: string
        confidence: number
        activeWindowJson: string | null
        createdAt: string
      }>).map((item) => ({
        id: item.id,
    ...
    getAgentPlans(limit = 30) {
      return (getAgentPlansStmt.all(limit) as Array<{
        id: string
        goal: string
        reasoning: string
        ...
        state: item.state,
        steps: JSON.parse(item.stepsJson) as AgentExecutionPlan['steps'],
        createdAt: item.createdAt,
    ...
    getAgentRuns(limit = 60) {
      return getAgentRunsStmt.all(limit) as AgentRunSummary[]
    },
    getAiSettings() {
      const rows = getAiSettingsRows.all() as Array<{ key: string; value: string; updatedAt: string }>
      const map = new Map(rows.map((row) => [row.key, row]))
      const updatedAt = rows[0]?.updatedAt ?? new Date().toISOString()
      return {
        preferredProvider: (map.get('preferredProvider')?.value as AiProviderSettings['preferredProvider']) ?? 'ollama',
    ...
    getAiProviderMetrics(limit = 120) {
      return getAiProviderMetricsStmt.all(limit) as AiProviderMetrics[]
    },
    upsertMemoryEmbedding(entry) {
      upsertMemoryEmbeddingStmt.run(
        entry.id,
        entry.kind,
        entry.content,
    ...
    semanticSearch(query, limit = 8) {
      ...
      const rows = getMemoryEmbeddingsStmt.all(300) as Array<{
        id: string
      ...
      return rows
        .map((item) => ({
          ...
          score: cosineSimilarity(queryVector, JSON.parse(item.vectorJson) as number[]),
          createdAt: item.createdAt,
        }))
    ...
    getLatestWorkspaceContext() {
      const row = getLatestWorkspaceContextStmt.get() as WorkspaceContext | undefined
      return row ?? null
    },
    ...
    getSkillEnabledStates() {
      const rows = getAiSettingsRows.all() as Array<{ key: string; value: string }>
      const map = new Map<string, boolean>()
      for (const row of rows) {
        if (!row.key.startsWith('skill.') || !row.key.endsWith('.enabled')) continue
    ...
    indexKnowledgeChunk(chunk, vector) {
      insertKnowledgeChunkStmt.run(
        chunk.id,
        chunk.sourceType,
        chunk.sourceRef,
        chunk.content,
        JSON.stringify(chunk.metadata),
        JSON.stringify(vector ?? createLocalEmbedding(chunk.content)),
        chunk.indexedAt,
      )
    },
    clearKnowledgeIndex() {
    ...
    semanticKnowledgeSearch(query, limit = 10, filter) {
      const queryVector = createLocalEmbedding(query)
      const rows = getKnowledgeChunksStmt.all(1200) as Array<{
        id: string
      ...
      return rows
        ...
        .map((item) => ({
          chunk: {
            ...
            content: item.content,
            metadata: JSON.parse(item.metadataJson) as Record<string, string>,
            indexedAt: item.indexedAt,
          },
          score: cosineSimilarity(queryVector, JSON.parse(item.embeddingJson) as number[]),
        }))
        .sort((a, b) => b.score - a.score)
        .slice(0, limit)
    },
    ...
    getKnowledgeGraphSnapshot(limit = 300) {
      const edges = getKnowledgeGraphEdgesStmt.all(limit) as KnowledgeGraphEdge[]
    ...
    addObservabilityEvent(event) {
      ...
      insertObservabilityEventStmt.run(
        created.id,
        created.type,
        created.source,
        created.severity,
    ...
    getRecentObservabilityEvents(limit = 120) {
      return (getRecentObservabilityEventsStmt.all(limit) as Array<{
        id: string
    ...
    addProactiveNotification(notification) {
      ...
      insertProactiveNotificationStmt.run(
        created.id,
        created.level,
        created.title,
        created.message,
    ...
    getRecentProactiveNotifications(limit = 80) {
      return getRecentProactiveNotificationsStmt.all(limit) as ProactiveNotification[]
    },
    ...
    addLearningFeedback(feedback) {
      ...
      insertLearningFeedbackStmt.run(
        created.id,
        created.type,
        created.source,
        created.action,
    ...
    getLearningFeedback(limit = 200) {
      return (getLearningFeedbackStmt.all(limit) as Array<{
        id: string
        type: LearningFeedbackRecord['type']
        source: LearningFeedbackRecord['source']
        action: string
        outcome: LearningFeedbackRecord['outcome']
        score: number
        metadataJson: string
        createdAt: string
      }>).map((item) => ({
    ...
    upsertBehaviorPattern(pattern) {
      upsertBehaviorPatternStmt.run(
        ...
        pattern.frequency,
        pattern.lastSeenAt,
        JSON.stringify(pattern.relatedActions),
      )
    },
    getBehaviorPatterns(limit = 50) {
      return (getBehaviorPatternsStmt.all(limit) as Array<{
        id: string
        name: string
        description: string
        category: BehaviorPattern['category']
        confidence: number
        frequency: number
        lastSeenAt: string
        relatedActionsJson: string
      }>).map((item) => ({
        ...item,
        relatedActions: JSON.parse(item.relatedActionsJson) as string[],
      }))
    },
    addAdaptiveRecommendation(recommendation) {
      ...
      insertAdaptiveRecommendationStmt.run(
        created.id,
        created.title,
        created.message,
        created.category,
    ...
    getAdaptiveRecommendations(limit = 60) {
      return getAdaptiveRecommendationsStmt.all(limit) as AdaptiveRecommendation[]
    },
    ...
    addWorkflowOptimizationInsight(insight) {
      ...
      insertWorkflowOptimizationInsightStmt.run(
        created.id,
        created.workflowId,
        created.workflowName,
        created.issue,
        created.suggestion,
        created.confidence,
        created.estimatedImpact,
        created.createdAt,
      )
      return created
    },
    getWorkflowOptimizationInsights(limit = 60) {
      return getWorkflowOptimizationInsightsStmt.all(limit) as WorkflowOptimizationInsight[]
    },
    getLearningSnapshot() {
    ...
    createDefaultMemoriesIfNeeded() {
      const now = new Date().toISOString()
      const workflowTotal = workflowCount.get() as { total: number }
      ...
      const scheduleTotal = (
        db.prepare('SELECT COUNT(*) as total FROM workflow_schedules').get() as { total: number }
      ).total
      if (scheduleTotal === 0) {
        const workflows = this.getWorkflows()
        const devWorkflow = workflows.find((item) => item.name.toLowerCase().includes('development'))
        if (devWorkflow) {
          insertWorkflowSchedule.run(
            randomUUID(),
            devWorkflow.id,
            'daily',
            '09:00',
      ...
      const projectTotal = projectCount.get() as { total: number }
      if (projectTotal.total === 0) {
        insertProject.run(
          ...
          JSON.stringify(['VS Code', 'Chrome']),
          now,
        )
      }

      const suggestionTotal = suggestionCount.get() as { total: number }
      if (suggestionTotal.total === 0) {
        insertSuggestion.run(
          randomUUID(),
          'You often launch VS Code before running npm dev.',
          0.82,
          now,
        )
        insertSuggestion.run(
          randomUUID(),
          'You usually check system usage after starting workflows.',
          0.67,
          now,
        )
      }
      const aiSettingsRows = getAiSettingsRows.all() as Array<{ key: string }>
