...
import type { WorkflowDefinition } from '../../shared/interfaces/ipc.js'
...
export class WorkflowEngine {
  ...
  async executeWorkflow(workflowId: string): Promise<{ ok: boolean; message: string }> {
  ...
  private stepToIntent(step: WorkflowDefinition['steps'][number]) {
  ...
  private validateWorkflow(workflow: WorkflowDefinition): { ok: boolean; message: string } {
    if (workflow.steps.length === 0) return { ok: false, message: 'Workflow has no steps.' }
    if (workflow.steps.length > 20) return { ok: false, message: 'Workflow exceeds max step limit.' }

    const orders = new Set<number>()
    for (const step of workflow.steps) {
      if (orders.has(step.order)) return { ok: false, message: 'Workflow has duplicate step order.' }
