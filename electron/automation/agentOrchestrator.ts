...
import type { AgentExecutionPlan, ParsedIntent } from '../../shared/interfaces/ipc.js'
...
export class AgentOrchestrator {
  ...
  async executePlan(
    planId: string,
    options: { allowRiskyActions?: boolean } = {},
    ...
    if (plan.steps.length > MAX_PLAN_DEPTH) return { ok: false, message: 'Plan exceeds max depth.' }
    if (!this.validateDependencies(plan)) return { ok: false, message: 'Plan has invalid dependencies.' }
    ...
    for (const step of pending) {
      ...
      for (let attempt = 0; attempt <= step.retryLimit; attempt += 1) {
        if (attempt > 0) this.memoryRepository.updateAgentPlanState(plan.id, 'retrying')
  ...
  private stepToIntent(actionType: AgentExecutionPlan['steps'][number]['actionType'], target: string): ParsedIntent {
    if (actionType === 'open_application') return parseIntent(`open ${target}`)
    if (actionType === 'open_url') return parseIntent(`open ${target}`)
    if (actionType === 'open_path') return parseIntent(`open ${target}`)
    return parseIntent(`run ${target}`)
  }

  private validateDependencies(plan: AgentExecutionPlan) {
