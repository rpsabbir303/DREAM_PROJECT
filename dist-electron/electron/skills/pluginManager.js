...
export class PluginManager {
    ...
    executeTool(payload) {
        ...
        if (!skill.enabled)
            return { ok: false, message: `Skill "${skill.name}" is disabled.` };
        const tool = skill.tools.find((item) => item.command === payload.toolCommand);
        if (!tool)
            return { ok: false, message: 'Tool not found in skill.' };
    ...
    hydrateFromMemory() {
        ...
        if (states.size === 0)
            return;
        this.skills = this.skills.map((skill) => states.has(skill.id) ? { ...skill, enabled: states.get(skill.id) ?? skill.enabled } : skill);
    }
}
