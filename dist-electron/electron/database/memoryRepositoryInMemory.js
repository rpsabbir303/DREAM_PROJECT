...
export function createInMemoryMemoryRepository() {
    ...
    const workflowOptimizationInsights = [];
    const aiKv = new Map();
    ...
    function saveOverlayStateImpl(state) {
    ...
    function getShortcutBindingsImpl() {
        return {
    ...
    function saveShortcutBindingsImpl(bindings) {
    ...
    function getIndexingStatusImpl() {
    ...
    function setIndexingStatusImpl(status) {
    ...
    function upsertMemoryEmbeddingInner(entry) {
        ...
        if (i >= 0)
            memoryEmbeddings[i] = row;
        else {
            memoryEmbeddings.unshift(row);
            if (memoryEmbeddings.length > 400)
                memoryEmbeddings.length = 400;
        }
    }
    function indexKnowledgeChunkInner(chunk, vector) {
        ...
        if (i >= 0)
            knowledgeChunks[i] = row;
        else
            knowledgeChunks.unshift(row);
    }
    const memoryRepo = {
        ...
        addAiProviderMetric(metric) {
            ...
            if (aiProviderMetrics.length > 500)
                aiProviderMetrics.length = 500;
        },
        ...
        addWorkspaceContext(context) {
            ...
            if (workspaceContexts.length > 200)
                workspaceContexts.length = 200;
        },
        ...
        getSkillEnabledStates() {
            ...
            for (const [key, value] of aiKv.entries()) {
                if (!key.startsWith('skill.') || !key.endsWith('.enabled'))
                    continue;
        ...
        upsertKnowledgeGraphEdge(edge) {
            ...
            if (i >= 0)
                knowledgeGraphEdges[i] = full;
            else
                knowledgeGraphEdges.unshift(full);
        },
        ...
        addObservabilityEvent(event) {
            ...
            if (observabilityEvents.length > 500)
                observabilityEvents.length = 500;
        ...
        addProactiveNotification(notification) {
            ...
            if (proactiveNotifications.length > 300)
                proactiveNotifications.length = 300;
            return created;
        },
        ...
        addLearningFeedback(feedback) {
            ...
            if (learningFeedback.length > 600)
                learningFeedback.length = 600;
            return created;
        },
