...
function severityBoost(chunk) {
    if (chunk.weightHints.severity === 'error')
        return 3;
    if (chunk.weightHints.severity === 'warning')
        return 1.5;
    return 0;
}
/** Rough char budget from approximate token target (heuristic). */
export function charsBudgetFromTokens(maxApproxTokens) {
    return Math.max(2000, maxApproxTokens * 4);
}
/**
 * Scores and packs context slices to respect a soft character budget.
 */
export function prioritizeContextChunks(chunks, userInput, preferences) {
    try {
