 ...
 */
function matchExactApp(rest) {
    const app = matchAppName(rest);
    if (!app)
        return null;
    const allowedPhrases = PHRASE_TO_APP.filter((p) => p.app === app).map((p) => p.phrase);
    const matchedPhrase = allowedPhrases.find((phrase) => normalise(rest) === phrase || normalise(rest).startsWith(`${phrase} `));
    if (!matchedPhrase)
        return null;
...
export function parseDesktopAutomationIntent(raw) {
 ...
 */
export function parseMultiIntent(raw) {
    const trimmed = normalise(raw);
    if (trimmed.length < 3)
        return null;
    let kind = null;
    ...
    for (const part of parts) {
        ...
        if (!app)
            return null; // any unrecognised app aborts the whole compound command
        intents.push(kind === 'open' ? { kind: 'open', app } : { kind: 'close', app });
    }
    return intents.length >= 2 ? intents : null;
}
export function displayNameForApp(app) {
    const labels = {
        chrome: 'Chrome',
        whatsapp: 'WhatsApp',
        vscode: 'VS Code',
