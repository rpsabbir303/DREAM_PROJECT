 ...
 */
function scoreWindow(query, w) {
    const q = norm(query);
    const proc = norm(w.processName);
    const title = norm(w.title);
    if (proc === q || title === q)
        return 100;
    if (proc.startsWith(q) && q.length >= 3)
        return 90;
    if (title.startsWith(q) && q.length >= 3)
        return 85;
    if (proc.includes(q) && q.length >= 3)
        return 75;
    if (title.includes(q) && q.length >= 3)
        return 70;
    if (q.includes(proc) && proc.length >= 3)
        return 68;
    if (q.includes(title) && title.length >= 3)
        return 65;
    ...
    if (qWords.length > 0) {
        ...
        if (matched.length === qWords.length)
            return 62;
        if (matched.length > 0)
            return Math.round(30 + (matched.length / qWords.length) * 30);
    }
    return 0;
}
/**
 ...
 */
export function findBestWindow(query) {
 ...
 */
export function findAllWindowsForApp(query) {
    const windows = getTrackedWindows();
...
`.trim();
async function runWin32(script) {
    const full = `${WIN32_BLOCK}; ${script}`.replace(/\n/g, ' ');
    const { stdout } = await execAsync(`powershell -NoProfile -NonInteractive -Command "${full.replace(/"/g, '\\"')}"`, { windowsHide: true, timeout: 8_000 });
    return stdout.trim();
}
/** Focus window by HWND. Restores it first if minimised. */
export async function focusByHwnd(hwnd) {
    try {
...
export async function closeByHwnd(hwnd) {
    try {
        // 0x0010 = WM_CLOSE
 ...
 */
export async function focusByQuery(query) {
    const win = findBestWindow(query);
    if (!win)
        return { ok: false, message: `No window found for "${query}".` };
    const result = await focusByHwnd(win.hwnd);
    return { ...result, hwnd: win.hwnd };
}
export async function minimizeByQuery(query) {
    const win = findBestWindow(query);
    if (!win)
        return { ok: false, message: `No window found for "${query}".` };
...
export async function maximizeByQuery(query) {
    const win = findBestWindow(query);
    if (!win)
        return { ok: false, message: `No window found for "${query}".` };
...
export async function restoreByQuery(query) {
    const win = findBestWindow(query);
    if (!win)
        return { ok: false, message: `No window found for "${query}".` };
...
export async function closeByQuery(query) {
    const win = findBestWindow(query);
    if (!win)
        return { ok: false, message: `No window found for "${query}".` };
