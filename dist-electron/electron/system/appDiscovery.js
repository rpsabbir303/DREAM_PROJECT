...
function buildAliases(name) {
    ...
    if (noVersion && noVersion !== base)
        aliases.add(noVersion);
...
function scoreMatch(query, candidate) {
    ...
    const matchedWords = qWords.filter((qw) => nameWords.some((nw) => nw.startsWith(qw) || qw.startsWith(nw)));
...
async function ps(script, timeoutMs = 20_000) {
    try {
        const safe = script.replace(/\n/g, '; ').replace(/"/g, '\\"');
        const { stdout } = await execAsync(`powershell -NoProfile -NonInteractive -Command "${safe}"`, { windowsHide: true, timeout: timeoutMs });
        return stdout.trim();
    }
    catch {
        return '';
...
function mergeRegistry(startMenu, registry, uwp, running) {
    ...
    for (const source of priority) {
        const list = source === 'start_menu' ? startMenu :
...
async function runFullScan() {
    ...
    _indexed = true;
    console.info(`[JARVIS_DISCOVERY] indexed ${_registry.length} apps ` +
...
export async function fuzzyResolve(query, minScore = 55) {
    ...
    if (best) {
        console.info(`[JARVIS_MATCH] resolved="${best.app.canonicalName}" ` +
...
export function getDiscoveryRegistry() {
    return _registry;
}
/** True once the first scan has completed. */
export function isDiscoveryReady() {
    return _indexed;
}
