...
export function resolveSpecialFolder(name) {
...
// ─── Open Folder ──────────────────────────────────────────────────────────────
export async function openFolder(nameOrPath) {
    ...
    if (err)
        return { ok: false, message: `Could not open folder: ${err}` };
    return { ok: true, message: `Opened ${special ? nameOrPath : targetPath}.` };
}
// ─── Create ───────────────────────────────────────────────────────────────────
export async function createFolder(name, parentDir) {
...
export async function createFile(name, parentDir, content = '') {
    const base = parentDir ?? path.join(os.homedir(), 'Desktop');
    const fullPath = path.join(base, name);
    try {
        ...
        if (msg.includes('EEXIST'))
            return { ok: false, message: `File "${name}" already exists.` };
...
export async function recycleFile(targetPath) {
    const resolved = path.resolve(targetPath);
    try {
...
export async function renameFile(targetPath, newName) {
    const resolved = path.resolve(targetPath);
 ...
 */
export async function searchFiles(query, searchRoot, maxResults = 20) {
    ...
    `;
        const { stdout } = await execAsync(`powershell -NoProfile -NonInteractive -Command "${ps.replace(/\n\s*/g, ' ')}"`, { windowsHide: true, timeout: 20_000 });
        const text = stdout.trim();
        if (!text || text === 'null')
            return { ok: true, results: [], message: `No files found matching "${query}".` };
...
export async function emptyRecycleBin() {
    try {
        await execAsync(`powershell -NoProfile -NonInteractive -Command "Clear-RecycleBin -Force -ErrorAction SilentlyContinue"`, { windowsHide: true, timeout: 15_000 });
