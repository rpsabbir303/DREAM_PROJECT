...
import { recordOpened, recordClosed, resolveContextualTarget, getPreviousAppName, getLastOpenedApp, } from '../../system/runtimeState.js';
...
import { isJunkDiscoveryName, isWhatsAppTypos, sanitizeAppOpenParams, } from '../../ai/nlp/appOpenResolver.js';
 ...
 */
async function liveMatchProcess(query) {
...
`;
        ...
        const { stdout } = await execAsync(`powershell -NoProfile -NonInteractive -ExecutionPolicy Bypass -File "${tmpFile}"`, { windowsHide: true, timeout: 8_000, maxBuffer: 4 * 1024 * 1024 });
        ...
        if (best)
            safeLogger.info(`[JARVIS_LIVE] matched "${query}" → "${best}" (score=${bestScore})`);
        else
            safeLogger.warn(`[JARVIS_LIVE] no live process match for "${query}"`);
        return bestScore >= 50 ? best : null;
    }
    catch (e) {
        safeLogger.error(`[JARVIS_LIVE] liveMatchProcess failed: ${e.message}`);
        return null;
    }
    finally {
 ...
 */
async function liveResolveExePath(appName) {
    const cleanName = appName.replace(/\.exe$/i, '');
    // 1. Try "where <name>"
    try {
        const { stdout } = await execAsync(`where "${cleanName}.exe"`, { windowsHide: true, timeout: 5_000 });
...
`;
        await writeFile(tmpFile, '\uFEFF' + script, 'utf-8');
        const { stdout } = await execAsync(`powershell -NoProfile -NonInteractive -ExecutionPolicy Bypass -File "${tmpFile}"`, { windowsHide: true, timeout: 6_000 });
...
function spawnExe(exePath, label) {
    try {
...
async function launchUwp(familyName, label) {
...
async function launchFromRegistryDef(def, label, appKey) {
...
async function openApp(appName, appKey, discoveryHint) {
    if (!isWindows())
        return { ok: false, message: 'App control is only supported on Windows.' };
...
// ─── Close ────────────────────────────────────────────────────────────────────
async function closeApp(appName, appKey) {
    if (!isWindows())
        return { ok: false, message: 'App control is only supported on Windows.' };
    const def = (appKey ? resolveApp(appKey) : null) ?? resolveApp(appName);
    const label = displayName(def, appName);
    ...
    if (def?.processNames?.length) {
        for (const n of def.processNames)
            knownNames.push(n);
    }
    ...
    const imageNames = [...new Set(knownNames.map((n) => n.toLowerCase().endsWith('.exe') ? n : `${n}.exe`))];
...
// ─── Focus ────────────────────────────────────────────────────────────────────
async function focusApp(appName, appKey) {
    const def = (appKey ? resolveApp(appKey) : null) ?? resolveApp(appName);
    ...
    const terms = [def?.windowTitle, appName].filter(Boolean);
...
async function minimizeApp(appName, appKey) {
    const def = (appKey ? resolveApp(appKey) : null) ?? resolveApp(appName);
...
async function maximizeApp(appName, appKey) {
    const def = (appKey ? resolveApp(appKey) : null) ?? resolveApp(appName);
    const query = def?.windowTitle ?? appName;
    const discovered = await fuzzyResolve(appName);
...
async function restoreApp(appName, appKey) {
    const def = (appKey ? resolveApp(appKey) : null) ?? resolveApp(appName);
    const query = def?.windowTitle ?? appName;
    const discovered = await fuzzyResolve(appName);
...
async function restartApp(appName, appKey) {
...
function resolveContextual(contextual) {
    switch (contextual) {
...
// ─── Plugin entry point ───────────────────────────────────────────────────────
export async function executeAppIntent(intent) {
    const contextual = intent.params.contextual;
    let app = intent.params.app ?? '';
    let appKey = intent.params.appKey;
    if (contextual) {
        const resolved = resolveContextual(contextual);
        if (!resolved) {
            const hint = contextual === 'active' ? "I'm not sure which app you're referring to. Which one?" :
    ...
    let discoveryName = intent.params.discoveryName;
    ...
    safeLogger.info(`[JARVIS_APP] executeAppIntent — type="${intent.type}" ` +
        `app="${app}" appKey="${appKey ?? ''}" discovery="${discoveryName ?? ''}"`);
    let result;
    switch (intent.type) {
        case 'app.open':
            result = await openApp(app, appKey, discoveryName);
            break;
        case 'app.close':
            result = await closeApp(app, appKey);
            break;
        case 'app.focus':
            result = await focusApp(app, appKey);
            break;
        case 'app.switch':
            result = await focusApp(app, appKey);
            break;
        case 'app.minimize':
            result = await minimizeApp(app, appKey);
            break;
        case 'app.maximize':
            result = await maximizeApp(app, appKey);
            break;
        case 'app.restore':
            result = await restoreApp(app, appKey);
            break;
        case 'app.restart':
            result = await restartApp(app, appKey);
            break;
