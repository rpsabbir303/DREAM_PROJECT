...
export function loadDesktopEnvironment() {
    ...
    if (rawKey && !usable) {
        console.warn('[JARVIS_ENV] OPENAI_API_KEY is set but ignored for OpenAI routing — use a real `sk-…` secret from https://platform.openai.com/api-keys (not template text).');
    }
    if (model) {
