...
import { runJarvisGeminiBasic, extractStructuredIntent, } from './geminiJarvisPipeline.js';
import { DESKTOP_INTENT_AUTO_EXECUTE, DESKTOP_INTENT_CONFIDENCE_FLOOR, routeIntent, } from './nlpRouter.js';
...
import { pushCommand, getSessionContext, runtimeState, } from '../system/runtimeState.js';
export class ChatEngine {
    ...
    async startStream(input, sender) {
        ...
        try {
            ...
            if (nlpResult && nlpResult.confidence >= DESKTOP_INTENT_CONFIDENCE_FLOOR) {
                ...
                const clarifyMsg = `Did you want me to **${nlpResult.displayLabel}**? ` +
            ...
            if (geminiDec.isOffline || !canUseConfiguredGemini()) {
                ...
                const keyLineEmpty = typeof process.env.GEMINI_API_KEY === 'string' &&
            ...
            const extracted = await extractStructuredIntent(input.input, { activeApp: runtimeState.activeWindow?.appName, recentCommands: ctx.recentCommands }, controller.signal);
            if (extracted) {
    ...
    async routeExtractedIntent(extracted, _nlpFallback, emit, input) {
    ...
    async executeLocalIntent(resolved, emit, streamId) {
        emit({
            streamId,
            type: 'provider',
            data: {
    ...
    // ─── Gemini conversation ─────────────────────────────────────────────────────
    async runGeminiChat(input, userMessage, emit, controller) {
        const ctx = getSessionContext();
...
// ─── Helpers ──────────────────────────────────────────────────────────────────
function emitLocal(emit, streamId, text) {
