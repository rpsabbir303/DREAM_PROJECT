 ...
 */
export async function fetchWithDeadline(url, init, deadlineMs, label = 'Request') {
...
const DEFAULT_STREAM_CHUNK_MS = Math.min(Math.max(45_000, Number(process.env.JARVIS_STREAM_CHUNK_READ_MS ?? 120_000)), 600_000);
/**
 * One `ReadableStreamDefaultReader.read()` bounded in time — fixes hung NDJSON/SSE bodies.
 */
export async function readStreamChunkWithTimeout(reader, chunkReadMs = DEFAULT_STREAM_CHUNK_MS) {
