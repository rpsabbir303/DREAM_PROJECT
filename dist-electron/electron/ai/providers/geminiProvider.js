import { GoogleGenerativeAI, GoogleGenerativeAIAbortError, GoogleGenerativeAIError, GoogleGenerativeAIFetchError, GoogleGenerativeAIRequestInputError, GoogleGenerativeAIResponseError, } from '@google/generative-ai';
import { getEffectiveGeminiApiKey, resolveGeminiModel } from '../geminiEnv.js';
import { readProcessEnv } from '../openAiEnv.js';
const GEMINI_TIMEOUT_MS = Math.min(Math.max(30_000, Number(process.env.GEMINI_FETCH_TIMEOUT_MS ?? process.env.OPENAI_FETCH_TIMEOUT_MS ?? 120_000)), 600_000);
function mapGeminiFailure(error) {
    ...
    if (error instanceof GoogleGenerativeAIFetchError) {
        ...
        if (s === 400)
            throw new Error(`Gemini bad request (HTTP 400): ${error.message}`);
        if (s === 401 || s === 403) {
