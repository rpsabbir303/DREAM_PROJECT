...
const OPENAI_FETCH_DEADLINE_MS = Math.min(Math.max(30_000, Number(process.env.OPENAI_FETCH_TIMEOUT_MS ?? 120_000)), 600_000);
/** Max wait per `read()` on the SSE body (stall / hung proxy). */
const STREAM_CHUNK_READ_MS = Math.min(Math.max(45_000, Number(process.env.JARVIS_STREAM_CHUNK_READ_MS ?? 120_000)), 600_000);
function getApiKey() {
    const apiKey = getEffectiveOpenAiApiKey();
    if (!apiKey) {
        const raw = readProcessEnv('OPENAI_API_KEY');
        if (!raw) {
            throw new Error('OPENAI_API_KEY is missing. Add a real key to the project root `.env` (Electron loads it via dotenv — not `.env.example`).');
        }
        throw new Error('OPENAI_API_KEY is set but does not look like a valid OpenAI secret key (expected `sk-…`). Replace template values in `.env` with your key from https://platform.openai.com/api-keys');
    }
    return apiKey;
}
...
function mapSdkFailure(error) {
    ...
    if (error instanceof APIError) {
        ...
        const detail = body && typeof body === 'object' && 'message' in body ? String(body.message) : error.message;
        if (status === 401) {
...
export function chatMessagesToOpenAi(messages) {
    return messages
 ...
 */
export async function completeOpenAiChat({ messages, model, tools, toolChoice = 'auto', signal, }) {
 ...
 */
export async function streamOpenAiChatRaw({ messages, model, onDelta, signal, }) {
    const apiKey = getApiKey();
    const targetModel = resolveModel(model);
    const response = await fetchWithDeadline('https://api.openai.com/v1/chat/completions', {
        ...
        signal,
    }, OPENAI_FETCH_DEADLINE_MS, 'OpenAI stream');
 ...
 */
export async function probeOpenAiConnectivity() {
